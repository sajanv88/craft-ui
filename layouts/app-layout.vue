...
<template>
  ...
  <section v-if="!loading">
    <section class="md:h-dvh md:py-1 md:px-2 flex">
      ...
      <UButton icon="i-lucide-menu" size="md" class="md:hidden m-2 w-10 h-10 flex items-center justify-center" @click="isSidebarOpen = !isSidebarOpen" />
      <aside :class="['fixed inset-y-0 left-0 transform bg-gray-800 text-white p-4 space-y-4 w-64 transition-transform', isSidebarOpen ? 'translate-x-0' : '-translate-x-full', 'md:relative md:translate-x-0']">
